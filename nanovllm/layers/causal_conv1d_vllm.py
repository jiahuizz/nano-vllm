...
# Copied from vllm.v1.attention.backends.utils
PAD_SLOT_ID = -1
# vLLM's PagedAttention reserves block 0 as the null/padding block, so slots with
# state_indices == 0 are skipped by the kernel. nano-vllm doesn't use that convention
# (slot 0 is a real sequence), so we disable null-block handling entirely by default.
NULL_BLOCK_ID = None
...
def causal_conv1d_fn(
    ...
    _causal_conv1d_fwd_kernel[grid](
        ...
        # others
        pad_slot_id,
        null_block_id if null_block_id is not None else 0,
        # META
